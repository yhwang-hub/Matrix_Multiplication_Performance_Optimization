...
__global__ void gemm_tiling_wpt_kernel(const int M, const int N, const int K, float* A, float* B, float* C)
{
    /* M×K * K×N = M×N */
    ...
    for(int ph = 0; ph < ceil(float(K) / float(TILE_WIDTH)); ++ph)
    {
        for (int w = 0; w < WPT; w++)
        {
            if(row < M && (ph * TILE_WIDTH + threadIdx.x + w * (TILE_WIDTH / WPT)) < K)
            {
                Ads[threadIdx.y][threadIdx.x + w * (TILE_WIDTH / WPT)] =
                    A[row * K + ph * TILE_WIDTH + threadIdx.x + w * (TILE_WIDTH / WPT)];
            }
            if(col + w * (TILE_WIDTH / WPT) < N && (ph * TILE_WIDTH + threadIdx.y) < K)
            {
                Bds[threadIdx.y][threadIdx.x + w * (TILE_WIDTH / WPT)] =
                    B[(ph * TILE_WIDTH + threadIdx.y) * N + col + w * (TILE_WIDTH / WPT)];
            }
        }
        __syncthreads();
        ...
        for (int i = 0; i < TILE_WIDTH; i++)
        {
            for (int w = 0; w < WPT; w++)
            {
                acc[w] += Ads[threadIdx.y][i] * Bds[i][threadIdx.x + w * (TILE_WIDTH / WPT)];
                //debug bank confilct
                /*int tid = threadIdx.y * (TILE_WIDTH/WPT) + threadIdx.x;
                if(tid < 32){
                    int ads_bank_idx = (threadIdx.y * TILE_WIDTH + i) ;//% 32;
                    int bds_bank_idx = (i * TILE_WIDTH + threadIdx.x + w*(TILE_WIDTH/WPT)) % 32;
    ...
    {
        if(row < M && col + w < N)
            C[row * N + col + w * (TILE_WIDTH / WPT)] = acc[w];
    }
}
...
void gemm_gpu_tiling_wpt(const int M, const int K, const int N, float* A, float* B, float* C, int nIter)
{
    ...
    /* 3. Launch kernel */
    dim3 dimBlock(TILE_WIDTH / WPT, TILE_WIDTH, 1);
    dim3 dimGrid(ceil(float(M) / float(dimBlock.x) / float(WPT)), ceil(float(N) / float(dimBlock.y)), 1);
    ...
    {
        gemm_tiling_wpt_kernel<<<dimGrid, dimBlock, 0>>>(
            M, K, N,
